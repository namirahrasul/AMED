...
def load_p2_weighing_model(config, ckpt, device, verbose=False):
    """Load the p2-weighing model (LWDM_Model) with specified configuration."""
    model = LWDM_Model(
        ...
        attention_resolutions=tuple(int(r) for r in config.model.attention_resolutions.split(',')),
        dropout=config.model.dropout,
        channel_mult=tuple(int(m) for m in config.model.channel_mult.split(',')),
...
def create_model(dataset_name=None, guidance_type=None, guidance_rate=None, device=None):
    ...
    if dataset_name in ['celeba-hq']:
        ...
        net = load_p2_weighing_model(config_path, model_path, device)
        net = P2Precond(
            net,
            img_resolution=config_path.model.image_size,
            img_channels=config_path.model.in_channels,
            guidance_rate=guidance_rate,
            guidance_type=guidance_type,
            label_dim=0,
            config=config_path  # Pass config for beta schedule
        ).to(device)
