...
@persistence.persistent_class
class P2Precond(nn.Module):
    """Preconditioning wrapper for p2-weighing model (UNetModel) for unconditional generation."""
    def __init__(self, model, img_resolution, img_channels, guidance_rate=0., guidance_type='uncond', label_dim=0, config=None):
        super().__init__()
        self.model = model
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        ...
        # Beta schedule from config
        if config and hasattr(config.diffusion, 'beta_start'):
            betas = np.linspace(config.diffusion.beta_start, config.diffusion.beta_end, 
                               config.diffusion.num_diffusion_timesteps, dtype=np.float64)
            alphas = 1.0 - betas
            self.alphas_cumprod = np.cumprod(alphas, axis=0)
            self.sqrt_alphas_cumprod = np.sqrt(self.alphas_cumprod)
            self.sqrt_one_minus_alphas_cumprod = np.sqrt(1.0 - self.alphas_cumprod)
            # Set sigma_min and sigma_max
            self.sigma_min = 0.002  # Standard AMED value
            self.sigma_max = float(np.sqrt((1 - self.alphas_cumprod[-1]) / self.alphas_cumprod[-1]))  # ≈ 4.459
            # For sigma and sigma_inv
            self.log_alpha_array = 0.5 * np.log(self.alphas_cumprod).reshape((1, -1))
            self.t_array = np.linspace(0., 1., len(self.alphas_cumprod) + 1)[1:].reshape((1, -1))
        else:
            raise ValueError("Config must provide diffusion beta schedule")

    def forward(self, x, sigma, labels=None, condition=None, unconditional_condition=None):
        sigma = sigma.view(-1, 1, 1, 1)
        # Map sigma to discrete timestep (0 to 999)
        device = x.device
        sigma_t = ((1.0 - self.alphas_cumprod) / self.alphas_cumprod) ** 0.5
        sigma_t = torch.from_numpy(sigma_t).float().to(device)
        t = torch.argmin(torch.abs(sigma_t.view(1, -1) - sigma.view(-1, 1, 1, 1)), dim=1).float()
        t = t * (1000.0 / len(sigma_t))  # Scale to 0-1000
        
        x = x.to(torch.float16 if self.config.model.use_fp16 else torch.float32)
        
        if self.guidance_type == 'uncond':
            out = self.model(x, t)
            out, _ = torch.split(out, 3, dim=1)  # Extract noise prediction
        else:
            raise NotImplementedError(f"Guidance type {self.guidance_type} not supported")
        
        return out

    def sigma(self, t):
        """Compute sigma for a given t (0 to 1)."""
        t = torch.as_tensor(t, device='cpu')
        log_alpha = self.interpolate_fn(t.reshape((-1, 1)), self.t_array, self.log_alpha_array)
        alpha = torch.exp(log_alpha).reshape(-1)
        return torch.sqrt((1.0 - alpha) / alpha).to(t.device)

    def sigma_inv(self, sigma):
        """Invert sigma to get t (0 to 1)."""
        sigma = torch.as_tensor(sigma, device='cpu')
        lamb = -(sigma.log())
        log_alpha = -0.5 * torch.logaddexp(torch.zeros(1), -2. * lamb)
        t = self.interpolate_fn(log_alpha.reshape((-1, 1)), torch.flip(self.log_alpha_array, [1]), torch.flip(self.t_array, [1]))
        return t.reshape(-1).to(sigma.device)
    ...
    @property
    def ema_scope(self):
        return contextlib.nullcontext()  # EMA handled by checkpoint
#----------------------------------------------------------------------------
...
class CFGPrecond(torch.nn.Module):
    ...
    @property
    def ema_scope(self):
        return self.model.ema_scope if hasattr(self.model, 'ema_scope') else contextlib.nullcontext()
